...
def main():
    ...
    input_path = base_dir / "romania_hiking.json"
    output_dir = base_dir / "hiking_docs"
    ...
    for trail in trails:
        ...
        t_asc = tags.get("ascent", "unknown")  # OSM tag pentru elevation gain
...
"""
        ...
        doc = [yaml_header]
        doc.append(f"# {name}")
        doc.append("Tip: Traseu de drumeție")
        ...
        if trail_lat and trail_lon:
            for poi in pois:
                ...
                if dist <= 5.0:
                    ...
                    if poi_tags.get("natural") == "spring":
                        poi_type = "Izvor natural (Apă)"
                    elif poi_tags.get("amenity") == "drinking_water":
                        poi_type = "Sursă de apă potabilă amenajată"
                    elif poi_tags.get("natural") == "peak":
                        poi_type = f"Vârf montan ({poi_tags.get('ele', '?')}m)"
                    elif poi_tags.get("emergency") == "mountain_rescue":
                        poi_type = "Bază / Refugiu Salvamont"
                    elif poi_tags.get("tourism") == "alpine_hut":
                        poi_type = "Cabană montană"
                    elif poi_tags.get("amenity") == "parking":
                        poi_type = "Parcare auto"
                    elif poi_tags.get("highway") == "via_ferrata":
                        poi_type = "Traseu expus / Via Ferrata"
        ...
        filename = f"{short_name}_{t_id}.md"

        with (output_dir / filename).open("w", encoding="utf-8") as out_f:
            out_f.write("\n".join(doc))
